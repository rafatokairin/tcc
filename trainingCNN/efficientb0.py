...
def load_synthetic_dataset(synth_root: Path):
    label_map = {"BENIGN": 0, "MALIGNANT": 1}
    filepaths, labels = [], []
    for p in synth_root.rglob("*"):
        if p.suffix.lower() in [".jpg", ".jpeg", ".png"]:
            prefix = p.stem.split("_")[0].upper()
...
def run_cross_validation(args, real_data, synth_data, n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X, y)):
        ...
        # Adicionar dados sintéticos apenas no treino
        X_train = np.concatenate([X_train_real, synth_data['filepath'].values])
        y_train = np.concatenate([y_train_real, synth_data['label'].values])
        
        # Criar datasets
        train_dataset = MammogramDataset(X_train.tolist(), y_train.tolist(), transform=transform_train)
        ...
        # Treinar só a cabeça primeiro (congela features, opcional)
...
def main():
    ...
    synth_df = load_synthetic_dataset(Path(args.synthetic_root))
    
    print(f"Real data: {len(reduced_df)} images")
    print(f"Synthetic data: {len(synth_df)} images")
    print(f"Class distribution - Real: {reduced_df['label'].value_counts().to_dict()}")
    print(f"Class distribution - Synthetic: {synth_df['label'].value_counts().to_dict()}")

    # Executar validação cruzada
    print(f"\nStarting {args.folds}-fold Cross-Validation...")
    results, models = run_cross_validation(args, reduced_df, synth_df, n_splits=args.folds)
